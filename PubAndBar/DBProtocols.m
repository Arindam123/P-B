...
@implementation DBHandeler (AditionalProtocol)

#define RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(type, sel)             \
va_list args;                                                        \
va_start(args, query);                                               \
ResultSet *resultSet = [self executeQuery:query withArgumentsInArray:0x00 orVAList:args];   \
va_end(args);                                                        \
...
- (BOOL)tableExists:(NSString*)tableName {
    ...
    ResultSet *rs = [self executeQuery:@"select [sql] from sqlite_master where [type] = 'table' and lower(name) = ?", tableName];
...
- (BOOL)columnExists:(NSString*)tableName columnName:(NSString*)columnName {
    ...
    //get table schema
    ResultSet *rs = [self getTableSchema: tableName];
