...
- (void)requestTokenFromProvider
{
    OAMutableURLRequest_LinkedIn *request = 
            [[[OAMutableURLRequest_LinkedIn alloc] initWithURL:requestTokenURL
                                             consumer:consumer
                                                token:nil   
    ...
    OADataFetcher_LinkedIn *fetcher = [[[OADataFetcher_LinkedIn alloc] init] autorelease];
...
//
- (void)requestTokenResult:(OAServiceTicket_LinkedIn *)ticket didFinish:(NSData *)data 
{
    ...
    self.requestToken = [[OAToken_LinkedIn alloc] initWithHTTPResponseBody:responseBody];
    [responseBody release];
    
    [self allowUserToLogin];
}

- (void)requestTokenResult:(OAServiceTicket_LinkedIn *)ticket didFail:(NSData *)error 
{
...
{ 
    OAMutableURLRequest_LinkedIn *request = 
            [[[OAMutableURLRequest_LinkedIn alloc] initWithURL:accessTokenURL
                                             consumer:consumer
...
- (void)accessTokenResult:(OAServiceTicket_LinkedIn *)ticket didFinish:(NSData *)data 
{
    NSString *responseBody = [[NSString alloc] initWithData:data
                                                   encoding:NSUTF8StringEncoding];
    ...
    {
        self.accessToken = [[OAToken_LinkedIn alloc] initWithHTTPResponseBody:responseBody];
        [self testApiCallForGroups];
    }
    [responseBody release];
}
...
{
    //NSURL *url = [NSURL URLWithString:@"https://api.linkedin.com/v1/people/~"];  ////for profile description
   // NSURL *url = [NSURL URLWithString:@"http://api.linkedin.com/v1/groups/4210332:(id,name,short-description,description,posts)"];/////for getting group discussions according to group id
    
    
   NSURL *url = [NSURL URLWithString:@"http://api.linkedin.com/v1/people/~/group-memberships:(group:(id,name),membership-state,show-group-logo-in-profile,allow-messages-from-members,email-digest-frequency,email-announcements-from-managers,email-for-every-new-post)"];
    
    //NSURL *url = [NSURL URLWithString:@"http://api.linkedin.com/v1/people/~/network/updates"];
    OAMutableURLRequest_LinkedIn *request = 
            [[OAMutableURLRequest_LinkedIn alloc] initWithURL:url
                                            consumer:consumer
    ...
    OADataFetcher_LinkedIn *fetcher = [[OADataFetcher_LinkedIn alloc] init];
    [fetcher fetchDataWithRequest:request
                         delegate:self
...
{
    //NSURL *url = [NSURL URLWithString:@"https://api.linkedin.com/v1/people/~"];  ////for profile description
     NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://api.linkedin.com/v1/groups/%d:(id,name,short-description,description,posts)",groupId]];/////for getting group discussions according to group id
    ...
    [fetcher fetchDataWithRequest:request
                         delegate:self
                didFinishSelector:@selector(testApiCallForGroupDiscussionsResult:didFinish:)
                  didFailSelector:@selector(testApiCallForGroupDiscussionsResult:didFail:)];    
    [request release];
}

- (void)testApiCallResult:(OAServiceTicket_LinkedIn *)ticket didFinish:(NSData *)data 
{
    NSString *responseBody = [[NSString alloc] initWithData:data
                                                   encoding:NSUTF8StringEncoding];
...
- (void)testApiCallResult:(OAServiceTicket_LinkedIn *)ticket didFail:(NSData *)error 
{
    NSLog(@"%@",[error description]);
}
- (void)testApiCallForGroupDiscussionsResult:(OAServiceTicket_LinkedIn *)ticket didFinish:(NSData *)data 
{
    NSString *responseBody = [[NSString alloc] initWithData:data
                                                   encoding:NSUTF8StringEncoding];
    ...
    // Notify parent and close this view
    [[NSNotificationCenter defaultCenter] 
     postNotificationName:@"groupDidSelected"        
     object:self
     userInfo:self.profile];
...
- (void)testApiCallForGroupDiscussionsResult:(OAServiceTicket_LinkedIn *)ticket didFail:(NSData *)error 
{
    NSLog(@"%@",[error description]);
}
//
...
{
    apikey = @"rqk1asokxrg4";//Put your API key here
    secretkey = @"e8DeBDOgG39OQf2t";//Put your secret key here   

    consumer = [[OAConsumer_LinkedIn alloc] initWithKey:apikey
                                        secret:secretkey
...
{
    if ([apikey length] < 12 || [secretkey length] < 16)
    {
