...
+ (id)initWithDBName:(NSString*)aName{
	...
	if(![self FileExist:DestinationPath])
	{
		...
		{
			return [[[self alloc] initWithPath:DestinationPath] autorelease];
		}
		else {
			return nil;
		}
...
- (ResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    @synchronized(self)
	{
		if (inUse) {
			[self compainAboutInUse];
